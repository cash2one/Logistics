...
class _LasyConnection(object):
    ...
    def commit(self):
        self.connection.commit() if self.connection else None
...
def _select(sql, first, *args):
    ...
    cursor = None
    sql = sql.replace('?', '%s')
    logging.info(('SQL: %s, ARGS: %s' % (sql, args)).decode("unicode-escape"))
    try:
...
if __name__ == '__main__':
    ...
    create_engine('fengdev', 'qaz123', 'test', host='10.0.0.212')
