        ...
        if key_name in mappings:
            ...
            if orm_val.ddl.find('int') != -1:
                try:
                    ...
                    raise ValueError("[%s]:[%s][%s] should be type of [%s]." % (key_name, str(kv), type(kv), orm_val.ddl))
                ...
                if (not kv) and orm_val.nullable is True:  # 参数值设置可以为空且传入参数就是空
                    continue
                elif not isinstance(kv, str) and not isinstance(kv, str):
                    raise ValueError("[%s]:[%s][%s] should be type of str." % (key_name, str(kv), type(kv)))
                elif kv and len(kv) > char_len:
                    raise ValueError("[%s]:[%s] should be str of length[%s]." % (key_name, str(kv), char_len))
        else:
            logging.warning("[%s]:[%s] won't be passed since [%s] is not valid." % (key_name, str(kv), key_name))