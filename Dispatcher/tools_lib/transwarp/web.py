...
class MultipartFile(object):
    ...
    def __init__(self, storage):
        self.filename = safe_unicode(storage.filename)
    ...
    def _parse_input(self):
        def _convert(item):
            if isinstance(item, list):
                return [safe_unicode(i.value) for i in item]
            if item.filename:
                return MultipartFile(item)
            return safe_unicode(item.value)
        ...
        # 不需要设置content_type == 'application/json', 直接判断body是否为空. 不为空就认为是json.
        # if request_method == 'POST' and request_body_size != 0:
        if request_method == 'POST' and str(content_type).find('application/json') != -1:
            request_body = self._environ['wsgi.input'].read(request_body_size)
            import json
            inputs = json.loads(request_body) if request_body else dict()
            if self.query_string:
                ...
                for pair in params:
                    ...
                    inputs[p[0]] = p[1]
            return inputs
        elif request_method == 'POST' and str(content_type) == 'text/xml':
            return {'xml': self._environ['wsgi.input'].read(request_body_size)}
...
class Response(object):
    ...
    def set_header(self, name, value):
        ...
        if not key in _RESPONSE_HEADER_DICT:
            key = name
        self._headers[key] = safe_str(value)
...
class WSGIApplication(object):
    ...
    def run(self, port=9000, host='127.0.0.1'):
        ...
        class StderrWSGIRequestHandler(WSGIRequestHandler):
            def log_message(self, format, *args):
                pass

        logging.info('application (%s) will start at %s:%s...' % (self._document_root, host, port))
        server = make_server(host, port, self.get_wsgi_application(debug=True), handler_class=WSGIRequestHandler)
        try:
            server.serve_forever()
        except:
            logging.info('application (%s) shuting down at %s:%s...' % (self._document_root, host, port))
            server.shutdown()
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def wsgi(env, start_response):
            ...
            try:
                ...
                content_length = ctx.response.content_length
                resp_status = ctx.response.status
                t2 = time.time()
                logging.info("[timeit] [%s] [%s][%s][%s bytes] [%s]: [%d msecs]" % (
                    env['REMOTE_ADDR'], env['REQUEST_METHOD'], env['PATH_INFO'], content_length, resp_status,
                    (t2 - t1) * 1000))
                del ctx.application
                del ctx.request
                del ctx.response
...
""">>>Begin Response Json API definition"""
...
def process_err(err, **kwargs):
    ctx.response.status = '400 %s' % str(err.__class__.__name__)
    # 错误返回信息
    request_method = ctx.request.request_method
    path_info = ctx.request.path_info
    ...
    if err.message:
        ...
        ret = dict(message='un-processable', request_method=request_method, path_info=path_info, headers=headers,
                   args=args)
        resp = dumps(ret)
    logging.exception("%s\n%s" % (ret, err.__repr__().decode('unicode-escape')))
    return resp
...
"""<<<End Response Json API definition"""
