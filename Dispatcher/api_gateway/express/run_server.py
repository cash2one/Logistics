...
from settings import settings, BL_DAS_API_PREFIX

define("port", default=4007, help="run on the given port", type=int)
...
class Application(tornado.web.Application):
    ...
    def log_request(self, handler):
        ...
        log_method("[timeit] [%s] [%s][%s][%s bytes] [%s]: [%d msecs][%s]" %
                   (handler.request.remote_ip, handler.request.method, handler.request.uri,
                    ...
                    handler.request.headers._dict.get('Authorization', '')))
...
if __name__ == "__main__":
    ...
    logging.info("I am calling at [%s]" % BL_DAS_API_PREFIX)
